...
import { deleteAllListings } from '../../services/storage/listingsStorage.js';
...
export default async function generalSettingsPlugin(fastify) {
  ...
  fastify.delete('/listings', async (request, reply) => {
    if (!isAdmin(request)) {
      return reply.code(403).send({ error: 'Only admins can reset the listing cache.' });
    }
    try {
      deleteAllListings();
      initSimilarityCache();
      logger.info('[Admin] All listings and hash cache cleared by admin reset.');
    } catch (err) {
      logger.error(err);
      return reply.code(500).send({ error: 'Error while resetting listing cache.' });
    }
    return reply.send();
  });
}

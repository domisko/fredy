...
const parseListingStatus = (row) => {
  ...
  if (typeof row.image_urls === 'string') {
    row.image_urls = fromJson(row.image_urls, null);
  }
  return row;
};
...
export const storeListings = (jobId, providerId, listings) => {
  ...
  SqliteConnection.withTransaction((db) => {
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, rooms, title, image_url, image_urls, description, address,
                             link, created_at, is_active, latitude, longitude, currency, translations, address_accuracy)
       VALUES (@id, @hash, @provider, @job_id, @price, @size, @rooms, @title, @image_url, @image_urls, @description, @address, @link,
               @created_at, 1, @latitude, @longitude, @currency, @translations, @address_accuracy)
    ...
    for (const item of listings) {
      ...
      const preloadedTranslations =
        item.translations && Object.keys(item.translations).length > 0 ? item.translations : null;
      const params = {
        id: nanoid(),
        hash: item.id,
        provider: providerId,
        ...
        translations: preloadedTranslations ? JSON.stringify(preloadedTranslations) : null,
        address_accuracy: item.addressAccuracy ?? null,
      };
      stmt.run(params);
...
/**
 * Hard-delete every listing across all jobs and users.
 * Use this to perform a full cache reset so that all providers re-notify on next run.
 *
 * @returns {any} The result from SqliteConnection.execute.
 */
export const deleteAllListings = () => SqliteConnection.execute('DELETE FROM listings');
